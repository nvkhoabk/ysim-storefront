...
const core = read(
  "src/lib/payments/payment-sandbox-execution-lifecycle-binding-issuance.ts",
);
const types = read(
  "src/lib/payments/payment-sandbox-execution-lifecycle-binding-issuance.types.ts",
);
const page = read(
  "src/app/ui-preview/payment-sandbox-execution-lifecycle-binding-issuance/page.tsx",
);
const registry = read(
  "src/i18n/payment-sandbox-execution-lifecycle-binding-issuance/payment-sandbox-execution-lifecycle-binding-issuance.registry.ts",
);
const i18nTypes = read(
  "src/i18n/payment-sandbox-execution-lifecycle-binding-issuance/payment-sandbox-execution-lifecycle-binding-issuance.types.ts",
);
const messages = {
  vi: read(
    "src/i18n/payment-sandbox-execution-lifecycle-binding-issuance/messages/vi.ts",
  ),
  en: read(
    "src/i18n/payment-sandbox-execution-lifecycle-binding-issuance/messages/en.ts",
  ),
  lo: read(
    "src/i18n/payment-sandbox-execution-lifecycle-binding-issuance/messages/lo.ts",
  ),
};
...
console.log(
  "PASS Vietnamese English and Lao execution-lifecycle-binding-issuance catalog parity",
);
...
console.log(
  "PASS Lao execution-lifecycle-binding-issuance UI catalog contains Lao Unicode",
);
...
console.log(
  "PASS locale switch preserves all execution-lifecycle-binding-issuance views and shell locale",
);
console.log(
  "PASS execution-lifecycle-binding-issuance navigation remains preview-only and query-based",
);

for (const marker of [
  "REUSE_F07A_3X_EXECUTION_LIFECYCLE_BINDING_WITHOUT_DUPLICATION",
  "BINDING_ISSUANCE_PREPARED_BUT_NEVER_PERSISTED_OR_SUBMITTED",
...
for (const marker of [
  ...
  "paymentExecutionLifecycleBindingIssuanceCreated: false",
...
console.log(
  "PASS execution-lifecycle-binding-issuance envelope remains unsigned non-secret non-PII and unsubmitted",
);
...
console.log(
  "PASS execution-lifecycle-binding-issuance preview does not inspect secrets call providers or mutate commerce",
);
...
console.log(
  "PASS: F07A-3Y payment sandbox execution lifecycle binding candidate contract.",
);
