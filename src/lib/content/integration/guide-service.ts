...
import type { ContentGateway } from "@/lib/content/refactor/content-gateway";
...
import { createGuideContentRuntime } from "./guide-runtime";

const supportedLocales: readonly ContentLocale[] = [
  "vi",
  "en",
  "lo",
  "ja",
...
function toGuideCard(
  ...
  const card = createArticleCardViewModel(source);
...
async function listWithFallback(
  ...
  locale: ContentLocale,
  category?: string,
): Promise<readonly WordPressContentSource[]> {
  const fallbackChain = contentLocaleFallbacks[locale];

  const byFamily = new Map<string, WordPressContentSource>();
  ...
  for (const candidateLocale of fallbackChain) {
    ...
    for (const item of items) {
      if (!byFamily.has(item.contentFamilyCode)) {
        byFamily.set(item.contentFamilyCode, item);
      }
    }
  }
...
async function getDetailWithFallback(
  ...
  for (const candidateLocale of fallbackChain) {
    const source = await gateway.getBySlug({
      kind: "guide",
...
export async function loadGuideLanding({
  ...
  locale?: ContentLocale;
  category?: string;
}): Promise<GuideLandingIntegrationResult> {
  const runtime = createGuideContentRuntime();

  const sources = await listWithFallback(runtime.gateway, locale, category);

  const sourceLabel =
    runtime.mode === "wordpress" ? "WordPress CMS" : "Fixture preview";
  ...
  return {
    ...
    page: {
      ...
      callout: {
        ...
        tone: runtime.mode === "wordpress" ? "success" : "info",
      },
    },
  };
}
...
export async function loadGuideArticle({
  ...
  locale?: ContentLocale;
  slug: string;
}): Promise<GuideArticleIntegrationResult | null> {
  const runtime = createGuideContentRuntime();

  const detail = await getDetailWithFallback(runtime.gateway, locale, slug);

  if (!detail) {
    return null;
  }

  const article = createArticlePageViewModel(detail.source);

  const allSources = await listWithFallback(runtime.gateway, locale);
  ...
  return {
    ...
    usedFallback: detail.resolvedLocale !== locale,
    ...
    page: {
      ...
      callout: {
        ...
        tone: detail.resolvedLocale !== locale ? "warning" : "info",
      },
    },
  };
}
