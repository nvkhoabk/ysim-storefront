...
import {
  createPaymentSandboxApprovalTokenTranslator,
  normalizePaymentSandboxApprovalTokenView,
} from "@/i18n/payment-sandbox-approval-token/payment-sandbox-approval-token.registry";
import type { PaymentSandboxApprovalTokenMessageKey } from "@/i18n/payment-sandbox-approval-token/payment-sandbox-approval-token.types";
...
} from "@/lib/payments/payment-sandbox-approval-token";
...
} from "@/lib/payments/payment-sandbox-approval-token.types";
...
const VIEWS: readonly PaymentSandboxApprovalTokenView[] = [
  "token",
  "eligibility",
  "token-envelope",
  "audit",
...
export default async function PaymentSandboxApprovalTokenPage({
  searchParams,
}: PreviewProps) {
  const params = await searchParams;
  const shell = createLocalizedShellBundle(params.locale);
  const view = normalizePaymentSandboxApprovalTokenView(params.view);
  const t = createPaymentSandboxApprovalTokenTranslator(shell.locale);
  ...
  const result = createPaymentSandboxApprovalToken(
  ...
  const audit = auditPaymentSandboxApprovalToken(
    result,
    upstreamApprovalIssuance,
    upstreamApproval,
    upstreamIssuance,
  ...
  const statusLabel = (status: PaymentSandboxApprovalTokenStatus): string => {
  ...
  const tabLabel = (candidate: PaymentSandboxApprovalTokenView): string => {
    if (candidate === "token") return t("tabs.token");
    if (candidate === "eligibility") return t("tabs.eligibility");
    if (candidate === "token-envelope") {
      return t("tabs.tokenEnvelope");
    }
    return t("tabs.audit");
  };
  ...
  const auditRows = [
    ...
    ["audit.tokenCandidateId", audit.checks.tokenCandidateIdMatches],
  ...
  return (
    <div lang={shell.htmlLang} dir={shell.direction}>
      <PageShell
        ...
        languageSwitch={{
          mode: "preview",
          previewPath: "/ui-preview/payment-sandbox-approval-token",
        }}
        homeHref={`/ui-preview/payment-sandbox-approval-token?locale=${shell.locale}&view=${view}`}
      >
            ...
            <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8">
              ...
              <aside className="rounded-3xl border border-white/20 bg-white/10 p-6">
                <dl className="grid gap-3 text-sm">
                  ...
                  />
                  <Row
                    label={t("summary.tokenCandidateId")}
                    value={result.approvalTokenCandidateId}
                  />
                  <Row
                ...
                >
                  {(["vi", "en", "lo"] as const).map((locale) => (
                    <Link
                      key={locale}
                      href={`/ui-preview/payment-sandbox-approval-token?locale=${locale}&view=${view}`}
                      aria-current={
          ...
          <section className="border-b border-slate-200 bg-white">
            ...
            >
              {VIEWS.map((candidate) => (
                <Link
                  key={candidate}
                  href={`/ui-preview/payment-sandbox-approval-token?locale=${shell.locale}&view=${candidate}`}
                    ...
                    {result.items.map((item) => (
                      ...
                      >
                        ...
                        <dl className="mt-5 grid gap-3 sm:grid-cols-2">
                          ...
                          />
                          <Row
                            label={t("field.approvalIssuanceId")}
                            value={item.approvalIssuanceId}
                          />
                          <Row
