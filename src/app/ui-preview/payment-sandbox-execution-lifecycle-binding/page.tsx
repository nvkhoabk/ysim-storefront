...
import {
  createPaymentSandboxExecutionLifecycleBindingTranslator,
  normalizePaymentSandboxExecutionLifecycleBindingView,
} from "@/i18n/payment-sandbox-execution-lifecycle-binding/payment-sandbox-execution-lifecycle-binding.registry";
import type { PaymentSandboxExecutionLifecycleBindingMessageKey } from "@/i18n/payment-sandbox-execution-lifecycle-binding/payment-sandbox-execution-lifecycle-binding.types";
...
import {
  auditPaymentSandboxExecutionLifecycleBinding,
  createPaymentSandboxExecutionLifecycleBinding,
} from "@/lib/payments/payment-sandbox-execution-lifecycle-binding";
...
} from "@/lib/payments/payment-sandbox-execution-lifecycle-binding.types";
...
export default async function PaymentSandboxExecutionLifecycleBindingPage({
  searchParams,
}: PreviewProps) {
  const params = await searchParams;
  const shell = createLocalizedShellBundle(params.locale);
  const view = normalizePaymentSandboxExecutionLifecycleBindingView(
    params.view,
  );
  const t = createPaymentSandboxExecutionLifecycleBindingTranslator(
    shell.locale,
  );
  ...
  const result = createPaymentSandboxExecutionLifecycleBinding(upstreamReceipt);
  const audit = auditPaymentSandboxExecutionLifecycleBinding(
    result,
    upstreamReceipt,
  );
  ...
  const statusLabel = (
    status: PaymentSandboxExecutionLifecycleBindingStatus,
  ): string =>
  ...
  const tabLabel = (
    candidate: PaymentSandboxExecutionLifecycleBindingView,
  ): string =>
    candidate === "binding"
      ? t("tabs.binding")
      : candidate === "lifecycle"
  ...
  const auditRows: readonly [
    ...
    [
      ...
      audit.checks.executionLifecycleBindingCandidateIdMatches,
    ],
  ...
  const route = "/ui-preview/payment-sandbox-execution-lifecycle-binding";
            ...
            <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
              ...
              <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                ...
                <Row
                  ...
                  value={result.executionLifecycleBindingCandidateId}
                />
                <Row
