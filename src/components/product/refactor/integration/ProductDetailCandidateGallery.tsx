...
import type { ProductDetailImageViewModel } from "@/types/view-models/product-detail-route-candidate";
...
export function ProductDetailCandidateGallery({
  ...
  images: readonly ProductDetailImageViewModel[];
}) {
  const t = createDetailTranslator(useStorefrontLocale().locale);
        ...
        <div className="mt-3 flex gap-3 overflow-x-auto pb-1">
          {images.map((image) => (
            <button
              key={image.id}
              type="button"
              aria-label={t("product.galleryViewImage", {
                name: image.alt,
              })}
