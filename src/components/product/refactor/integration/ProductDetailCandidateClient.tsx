...
import { useWooCommerceCart } from "@/features/cart/refactor";
...
import { ProductDetailCandidateGallery } from "./ProductDetailCandidateGallery";
...
export function ProductDetailCandidateClient({
  ...
  candidate: ProductDetailRouteCandidateViewModel;
  showDiagnostics?: boolean;
  diagnostics?: React.ReactNode;
}) {
  const { locale } = useStorefrontLocale();
  const t = createDetailTranslator(locale);
  const shellT = createShellTranslator(locale);
  const product = candidate.product;

  const cartState = useWooCommerceCart();

  const matrix = useMemo(
    () => createProductDetailVariantMatrix(product.variations),
    [product.variations],
  );
  ...
  const [selection, setSelection] = useState<Readonly<Record<string, string>>>(
    () =>
      createInitialProductVariantSelection({
        ...
        dimensions: matrix.primaryDimensions,
      }),
  );
  ...
  const selected = useMemo(
    () =>
      findSelectedProductVariation(product.variations, selection) ||
  ...
  function chooseOption(dimensionKey: string, optionValue: string) {
  ...
  async function addCandidateSelection() {
    if (!selected || !canAdd) {
      setFeedback(t("product.unavailableFeedback"));
      return;
    }

    const cartSelection: ProductDetailCartSelection = {
      productId: product.id,
      productSlug: product.slug,
      productName: product.name,
      variationId: selected.id,
      variationSku: selected.sku,
      quantity,
    ...
    try {
      await cartState.add({
        productId: product.id,
        variationId: selected.id !== product.id ? selected.id : undefined,
        quantity,
        variation: Object.entries(selected.attributes).map(
          ([attribute, value]) => ({
            attribute,
            value,
          }),
      ...
      window.dispatchEvent(
        new CustomEvent("ysim:product-candidate:add-to-cart", {
          detail: cartSelection,
        }),
      );

      setFeedback(t("product.addedFeedback", { selection: selected.label }));
    } catch (caught) {
      setFeedback(
        caught instanceof Error ? caught.message : t("product.addError"),
      );
    }
  }

  return (
    <PageShell cartCount={cartState.cart?.items_count || 0}>
        ...
        <Container>
          ...
          >
            ...
            <span className="text-[var(--ysim-color-text)]">
              {product.destinationName || product.name}
            </span>
          </nav>
        </Container>
      </Section>
      ...
      <Section spacing="lg">
        <Container>
          <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(22rem,0.9fr)]">
            <ProductDetailCandidateGallery images={product.gallery} />

            <div className="lg:sticky lg:top-28">
              {product.destinationName ? (
                <p className="text-sm font-bold tracking-[0.1em] text-[var(--ysim-color-brand-700)] uppercase">
                  {product.destinationName}
                </p>
              ) : null}
              ...
              <div className="mt-6 rounded-[var(--ysim-radius-xl)] border border-[var(--ysim-color-border)] bg-white p-5 shadow-[var(--ysim-shadow-sm)]">
                ...
                {matrix.primaryDimensions.length > 0 ? (
                      ...
                      <fieldset key={dimension.key}>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {dimension.options.map((option) => {
                            const active =
                              selection[dimension.key] === option.value;
                            ...
                            return (
                              <button
                                ...
                                disabled={!available}
                                onClick={() =>
                                  chooseOption(dimension.key, option.value)
                                }
                                className={cn(
                  ...
                  <div className="mt-5 rounded-[var(--ysim-radius-lg)] bg-[var(--ysim-color-surface-subtle)] p-4">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        ...
                        {!selected.inStock || !selected.purchasable ? (
                      ...
                      <div className="text-right">
                        ...
                        <Price
                          ...
                          originalAmount={selected.regularPrice}
                        />
                      </div>
                    </div>
                  </div>
                      ...
                      <Price
                        ...
                        originalAmount={
                          selected.regularPrice
                            ? selected.regularPrice * quantity
                            : undefined
                        }
                      />
                    </div>
                ...
                <button
                  type="button"
                  disabled={!canAdd || cartState.pending === "add"}
                  onClick={() => void addCandidateSelection()}
      ...
      <Section variant="subtle">
        <Container>
          <SectionHeader
            ...
            title={t("product.overviewTitle")}
          />

          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {product.features.map((feature, index) => {
              const icons = [Database, Signal, Globe2];
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-4 text-xs font-bold tracking-[0.08em] text-[var(--ysim-color-text-soft)] uppercase">
                    {feature.label}
                  </p>
      ...
      <Section>
        <Container>
          <SectionHeader
            eyebrow={t("product.usageEyebrow")}
            title={t("product.usageTitle")}
          />
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {product.usageNotes.map((note) => (
              <p
                key={note}
                className="flex items-start gap-3 rounded-[var(--ysim-radius-lg)] border border-[var(--ysim-color-border)] bg-white px-4 py-4 text-sm leading-relaxed font-semibold text-[var(--ysim-color-text-muted)]"
              >
      ...
      {product.relatedProducts.length > 0 ? (
        <Section variant="subtle">
          <Container>
            <SectionHeader
              eyebrow={t("product.relatedTitle")}
              title={t("product.relatedDescription")}
            />
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {product.relatedProducts.map((item) => (
                <Link
                  key={item.id}
                  href={localizeShellHref(`/esim/${item.slug}`, locale)}
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 text-xs font-semibold text-[var(--ysim-color-text-muted)]">
                      {item.dataLabel ? <span>{item.dataLabel}</span> : null}
