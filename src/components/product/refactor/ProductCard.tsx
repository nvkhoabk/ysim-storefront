...
import { ProductAttributeChips } from "./ProductAttributeChips";
...
const badgeIconMap: Record<ProductBadgeIcon, LucideIcon> = {
  featured: Sparkles,
...
export function ProductCard({
  ...
  const dataLabel = localizeDataLabel(product.dataLabel, locale);
  const durationLabel =
    localizeDurationLabel(product.durationLabel, locale) ||
  ...
  return (
    ...
    >
      <Link
        href={product.href}
        aria-label={t("ordinary.productLinkAria", { name: product.name })}
        ...
        {product.badges.length > 0 ? (
          <div className="absolute top-4 left-4 flex flex-wrap gap-2">
            {product.badges.map((badge) => {
              const Icon = badge.icon ? badgeIconMap[badge.icon] : null;
