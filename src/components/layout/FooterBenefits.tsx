...
const footerBenefits: FooterBenefitItem[] = [
  {
    ...
    description: "Nhận eSIM và kết nối chỉ trong vài phút.",
    icon: Clock3,
  },
  {
    title: "Phủ sóng toàn cầu",
    description: "Hơn 200 quốc gia và vùng lãnh thổ.",
    icon: Globe2,
  },
  {
    title: "Kết nối an toàn",
    description: "Hạ tầng bảo mật cao, bảo vệ dữ liệu tuyệt đối.",
    icon: ShieldCheck,
  },
  {
    title: "Hỗ trợ 24/7",
...
export function FooterBenefits() {
  return (
    <section className="bg-white px-5 pt-2 pb-6 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl grid-cols-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:grid-cols-4">
        {footerBenefits.map((benefit, index) => {
          ...
          return (
            <article
              key={benefit.title}
              className={`relative flex min-h-[84px] items-center gap-3 px-4 py-3 sm:gap-4 sm:px-5 lg:min-h-[92px] lg:px-6 ${
            ...
            >
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-transparent text-green-700 sm:h-10 sm:w-10">
                <Icon
                  aria-hidden="true"
              ...
              <div className="min-w-0">
                <h2 className="text-[14px] leading-5 font-bold text-slate-950 sm:text-[15px]">
                  {benefit.title}
                </h2>
