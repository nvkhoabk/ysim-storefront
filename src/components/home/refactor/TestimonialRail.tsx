...
import type { TestimonialSectionViewModel } from "@/types/view-models/home";
...
export interface TestimonialRailProps {
  section: TestimonialSectionViewModel;
}

export function TestimonialRail({ section }: TestimonialRailProps) {
  ...
  return (
    <Section variant="subtle">
      <Container>
        <SectionHeader
          ...
          action={
            <div className="hidden items-center gap-2 sm:flex">
              <button
                type="button"
                aria-label={t("ordinary.previousReviews")}
              ...
              <button
                type="button"
                aria-label={t("ordinary.nextReviews")}
                onClick={() => scroll("next")}
                className="inline-flex h-10 w-10 items-center justify-center rounded-[var(--ysim-radius-md)] border border-[var(--ysim-color-border)] bg-white transition-[transform,border-color] hover:-translate-y-0.5 hover:border-[var(--ysim-color-brand-300)]"
              >
        ...
        <div
          ref={railRef}
          className="grid snap-x snap-mandatory [scrollbar-width:none] auto-cols-[minmax(18rem,84vw)] grid-flow-col gap-5 overflow-x-auto pb-4 sm:auto-cols-[minmax(20rem,24rem)] [&::-webkit-scrollbar]:hidden"
        >
