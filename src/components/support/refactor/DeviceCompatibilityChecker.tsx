...
import { DeviceCheckGuide } from "./DeviceCheckGuide";

export interface DeviceCompatibilityCheckerProps {
  devices: readonly DeviceCompatibilityViewModel[];
  manualChecks: readonly DeviceManualCheckViewModel[];
}
...
export function DeviceCompatibilityChecker({
  ...
  const brands = useMemo(
    () => Array.from(new Set(devices.map((device) => device.brand))).sort(),
    [devices],
  );
  ...
  const visibleDevices = useMemo(() => {
    ...
    return devices.filter((device) => {
      const matchesBrand = !brand || device.brand === brand;
  ...
  const selectedDevice = devices.find((device) => device.id === modelId);
  ...
  return (
    <Section id="device-compatibility" variant="subtle">
      <Container>
        <SectionHeader
          eyebrow={copy.sections.deviceEyebrow}
          title={copy.sections.deviceTitle}
          description={copy.sections.deviceDescription}
        />
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(20rem,0.95fr)]">
          <div className="rounded-[var(--ysim-radius-xl)] border border-[var(--ysim-color-border)] bg-white p-5 shadow-[var(--ysim-shadow-sm)] sm:p-6">
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <label className="block">
                ...
                <select
                  ...
                  className="min-h-12 w-full rounded-[var(--ysim-radius-md)] border border-[var(--ysim-color-border-strong)] bg-white px-3.5 text-sm font-semibold outline-none focus:border-[var(--ysim-color-brand-600)] focus:ring-4 focus:ring-[var(--ysim-color-brand-100)]"
                >
                  <option value="">{copy.controls.chooseModel}</option>
            ...
            <label className="mt-5 block">
              ...
              <span className="relative block">
                <Search className="pointer-events-none absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-[var(--ysim-color-text-soft)]" />

                <input
                  value={query}
                  onChange={(event) => setQuery(event.target.value)}
                  placeholder={copy.controls.searchPlaceholder}
                  className="min-h-12 w-full rounded-[var(--ysim-radius-md)] border border-[var(--ysim-color-border-strong)] bg-white pr-3.5 pl-10 text-sm font-semibold outline-none focus:border-[var(--ysim-color-brand-600)] focus:ring-4 focus:ring-[var(--ysim-color-brand-100)]"
                />
              </span>
            </label>
            ...
            <div className="mt-6">
              {selectedDevice ? (
                <CompatibilityResultCard device={selectedDevice} />
              ) : (
          ...
          <DeviceCheckGuide steps={manualChecks} />
        </div>
      </Container>
    </Section>
  );
