...
import { PaymentResultPageComposition } from "@/components/payment/refactor";

import { createVerifiedPaymentCandidate } from "@/features/payments/candidate/payment-candidate-client";

import { getPaymentProviderLabel } from "@/features/payments/candidate/payment-provider-presentation";

import { presentPaymentCandidateResult } from "@/features/payments/candidate/payment-result-presenter";
...
import { PaymentCandidateDiagnostics } from "./PaymentCandidateDiagnostics";
...
const sessionStorageKey = "ysim-payment-candidate-session";

function providerLabel(provider: PaymentProviderId): string {
  return getPaymentProviderLabel(provider);
}
...
function parseHandoff(value: string | null): CheckoutOrderHandoff | null {
  ...
  try {
    ...
    if (!parsed.orderId || !parsed.orderKey || !parsed.provider) {
      return null;
    }
...
function parseStoredSession(
  ...
  try {
    const parsed = JSON.parse(value) as PaymentCandidateStoredSession;

    return parsed.session && parsed.handoff && parsed.order ? parsed : null;
  } catch {
    return null;
  }
}
...
export function PaymentCandidateClient({
  ...
  config: PaymentCandidateConfigViewModel;
  resultOnly?: boolean;
  ...
  const [stored, setStored] = useState<PaymentCandidateStoredSession | null>(
    null,
  );
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      setHandoff(
        parseHandoff(window.sessionStorage.getItem(handoffStorageKey)),
      );

      setStored(
        parseStoredSession(window.sessionStorage.getItem(sessionStorageKey)),
      );
  ...
  const providerEnabled = useMemo(
    () =>
      handoff ? config.enabledProviders.includes(handoff.provider) : false,
    [config.enabledProviders, handoff],
  );

  async function createPayment() {
    if (!handoff || creating || !providerEnabled) {
      return;
    }
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("ysim:payment-candidate-created", {
          detail: nextStored,
        }),
      );
  ...
  if (loading) {
    return (
      <PageShell cartCount={0}>
        ...
        {showDiagnostics ? (
          <PaymentCandidateDiagnostics config={config} />
        ) : null}
      </PageShell>
    );
  }

  if (resultOnly && !stored) {
    return (
      <PageShell cartCount={0}>
        <Section spacing="lg">
          <Container size="content">
            <div className="rounded-[var(--ysim-radius-xl)] border border-[var(--ysim-color-border)] bg-white p-9 text-center shadow-[var(--ysim-shadow-sm)]">
              ...
              <h1 className="mt-5 text-3xl font-bold text-[var(--ysim-color-text)]">
                {t("payment.sessionMissingTitle")}
              </h1>

              <p className="mt-3 text-sm text-[var(--ysim-color-text-muted)]">
                {t("payment.sessionMissingDescription")}
              </p>
            ...
            <div className="flex flex-col gap-3 rounded-[var(--ysim-radius-lg)] border border-[var(--ysim-color-border)] bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs font-semibold text-[var(--ysim-color-text-muted)]">
                  Order #{stored.order.orderNumber} ·{" "}
                  {stored.order.customerEmailMasked}
                </p>
              </div>

              <div className="flex flex-wrap gap-2">
                {stored.session.redirectUrl ? (
                  <a
                    href={stored.session.redirectUrl}
                    target="_blank"
                    rel="noreferrer"
  ...
  return (
    <PageShell cartCount={0}>
        ...
        <Container size="content">
          <div className="rounded-[var(--ysim-radius-xl)] border border-[var(--ysim-color-border)] bg-white p-6 shadow-[var(--ysim-shadow-sm)] sm:p-8">
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-xs font-bold tracking-[0.08em] text-[var(--ysim-color-text-soft)] uppercase">
                  Order handoff
                </p>
            ...
            <dl className="mt-6 grid gap-4 rounded-[var(--ysim-radius-lg)] bg-[var(--ysim-color-surface-subtle)] p-5 sm:grid-cols-2">
              <div>
                <dt className="text-xs font-bold tracking-[0.08em] text-[var(--ysim-color-text-soft)] uppercase">
                  Phương thức
                </dt>
                <dd className="mt-1 font-bold text-[var(--ysim-color-text)]">
                  {providerLabel(handoff.provider)}
                </dd>
              </div>
              ...
              <div>
                ...
                <dd className="mt-1">
                  <Price amount={handoff.amount} size="compact" />
                </dd>
              </div>
              ...
              <div>
                ...
                <dd className="mt-1 font-bold break-all text-[var(--ysim-color-text)]">
            ...
            <div className="mt-6 rounded-[var(--ysim-radius-lg)] border border-[var(--ysim-color-brand-200)] bg-[var(--ysim-color-brand-50)] p-4">
              <p className="flex items-start gap-3 text-sm leading-relaxed font-semibold text-[var(--ysim-color-brand-900)]">
                ...
                Amount hiển thị ở trên chỉ là tham chiếu. Server sẽ đọc lại
                total, currency và billing từ WooCommerce trước khi tạo Payment
                Session.
              </p>
            </div>
            ...
            {!providerEnabled ? (
              <div className="mt-5 rounded-[var(--ysim-radius-md)] border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900">
                Provider {handoff.provider} chưa nằm trong
            ...
            <button
              type="button"
              disabled={creating || !providerEnabled}
            ...
            <p className="mt-3 text-center text-xs leading-relaxed font-semibold text-[var(--ysim-color-text-soft)]">
      ...
      {showDiagnostics ? <PaymentCandidateDiagnostics config={config} /> : null}
    </PageShell>
  );
}
