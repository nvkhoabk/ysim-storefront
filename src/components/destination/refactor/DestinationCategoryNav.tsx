...
export interface DestinationCategoryNavProps {
  items: readonly DestinationCategoryOptionViewModel[];
  value: DestinationContinentKey;
  onChange: (value: DestinationContinentKey) => void;
}
...
export function DestinationCategoryNav({
  ...
  return (
    <nav
      aria-label={t("destinations.regionFilter")}
      className="flex [scrollbar-width:none] gap-2 overflow-x-auto pb-2 [&::-webkit-scrollbar]:hidden"
    >
      {items.map((item) => {
