...
import type { WooCommerceCartItem } from "@/lib/woocommerce/cart-types";

import type { CartRouteCandidateViewModel } from "@/types/view-models/cart-route-candidate";

import { CartRouteCandidateNotice } from "./CartRouteCandidateNotice";
...
function amount(value: string | undefined, minorUnit: number): number {
  const raw = Number(value || 0);

  return Number.isFinite(raw) ? raw / Math.pow(10, minorUnit) : 0;
}

function variationLabels(item: WooCommerceCartItem): readonly string[] {
  return [
    ...(item.variation || []).map(
      (attribute) => `${attribute.attribute}: ${attribute.value}`,
    ),
    ...(item.item_data || []).map(
      (data) => `${data.key}: ${data.display || data.value}`,
    ),
  ];
}
...
export function CartCandidateClient({
  ...
  candidate: CartRouteCandidateViewModel;
  ...
  if (cartState.isLoading) {
    return (
      <PageShell cartCount={0}>
        ...
        {showDiagnostics ? (
          <CartRouteCandidateNotice candidate={candidate} />
        ) : null}
      </PageShell>
    );
  }
          ...
          <Container size="content">
            <div className="rounded-[var(--ysim-radius-xl)] border border-red-200 bg-red-50 p-8 text-center">
              <h1 className="text-2xl font-bold text-red-900">
                {t("cart.loadErrorTitle")}
              </h1>

              <p className="mt-3 text-sm font-semibold text-red-800">
                {cartState.error || t("cart.loadErrorDescription")}
              </p>

              <button
                type="button"
                onClick={() => void cartState.reload()}
  ...
  return (
    <PageShell cartCount={cart.items_count}>
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
            <div className="space-y-4">
              {cart.items.map((item) => {
                ...
                const isPending = cartState.pending === item.key;

                const labels = variationLabels(item);
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-5 flex flex-wrap items-center justify-between gap-4">
                        ...
                        <button
                          type="button"
                          disabled={isPending}
                          onClick={() => void cartState.remove(item.key)}
            ...
            <aside className="h-fit rounded-[var(--ysim-radius-xl)] border border-[var(--ysim-color-border)] bg-white p-6 shadow-[var(--ysim-shadow-sm)] lg:sticky lg:top-28">
              ...
              <dl className="mt-5 space-y-3 text-sm">
                <div className="flex justify-between gap-4 text-[var(--ysim-color-text-muted)]">
                  ...
                  <dd className="font-semibold">
                    <Price
                      amount={amount(cart.totals.total_items, minorUnit)}
                      size="compact"
                    />
                  </dd>
                </div>

                {amount(cart.totals.total_discount, minorUnit) > 0 ? (
              ...
              <div className="mt-5 border-t border-[var(--ysim-color-border)] pt-5">
                <div className="flex items-end justify-between gap-4">
                  ...
                  <Price amount={amount(cart.totals.total_price, minorUnit)} />
                </div>
              </div>
              ...
              <div className="mt-6 border-t border-[var(--ysim-color-border)] pt-5">
                ...
                <div className="mt-3 flex gap-2">
                  ...
                  <button
                    type="button"
                    disabled={
                      cartState.pending === "coupon" || !couponCode.trim()
                    }
                    ...
                    {cart.coupons.map((coupon) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          disabled={cartState.pending === "coupon"}
                          onClick={() =>
                            void cartState.removeCoupon(coupon.code)
                          }
                        >
                          <X className="h-4 w-4" />
                        </button>
