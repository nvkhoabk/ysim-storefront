...
function resultStatus(status: PaymentStatus): PaymentResultStatus {
  switch (status) {
    case "paid":
      return "success";
...
function statusCopy(session: PaymentSession): {
  label: string;
  title: string;
  description: string;
} {
  const status = resultStatus(session.status);
  ...
  if (status === "processing") {
    return {
      label: "Đang khởi tạo",
      title: "Đang chuẩn bị phiên thanh toán",
      description:
        session.message ||
...
export function presentPaymentCandidateResult({
  ...
  return {
    result: {
      status: resultStatus(session.status),
      statusLabel: copy.label,
      orderCode: session.orderNumber,
      title: copy.title,
      description: copy.description,
      paymentMethodId: mapPaymentProviderToCheckoutMethod(session.provider),
      paymentMethodLabel: providerLabel(session.provider),
      amount: session.amount,
      customerEmail: handoff.customerEmail,
      recipientEmail: handoff.recipientEmail,
      createdAtLabel: new Date(handoff.createdAt).toLocaleString("vi-VN"),
      providerReference:
        session.providerTransactionId || session.merchantTransactionId,
      ...
      actions: [
        {
          ...
          href: localizeShellHref(
            `/orders/${encodeURIComponent(handoff.orderNumber)}`,
            locale,
          ),
          variant: "secondary",
        },
